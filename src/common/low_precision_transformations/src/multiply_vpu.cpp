...
MultiplyVpuTransformation::MultiplyVpuTransformation(const Params& params) : EltwiseBaseTransformation(params) {
    MATCHER_SCOPE(MultiplyVpuTransformation);
...
bool MultiplyVpuTransformation::transform(TransformationContext& context, ngraph::pattern::Matcher &m) {
    ...
    const int emptyPathIndex = fullPathIndex == 0 ? 1 : 0;
    {
        ...
        inputs[fullPathIndex] = dequantizationFullPath.data;
...
bool MultiplyVpuTransformation::canBeTransformed(const TransformationContext& context, std::shared_ptr<Node> layer) const {
